...
class NoteAnimationModel:
    ...
    def _calculate_ticks_per_second(self):
        """Calculate how many ticks occur in one second based on BPM and ticks per beat."""
        beats_per_second = self.bpm / 60  # Convert BPM to BPS (Beats Per Second)
        return beats_per_second * self.ticks_per_beat

    def _calculate_frames_per_tick(self):
    ...
    def get_total_number_of_frames(self, events):
    ...
    def get_note_position(self, start_tick, current_frame, piano_height=185):
        """
    ...
    def _binary_search(self, note_events, start_tick):
        """
    ...
    def get_active_note_events(
        ...
        # find the index of the first note that has not started yet with binary search
    ...
    def get_total_ticks(self, midi_file):
        return midi_file.ticks_per_beat * (self.bpm / 60) * midi_file.length

    def get_note_length(self, duration):
